...
2. pact-cli: `pact-broker record-deployment --pacticipant=pact-order --version=$(git rev-parse HEAD) --environment=dev`
3. docker:
```
docker run --rm \
...
1. gradle: At time of writing the pact gradle plugin does not support recording a deployment
2. pact-cli: `pact-broker record-release --pacticipant=pact-order --version=$(git rev-parse HEAD) --environment=prod`
3. docker:
```
docker run --rm \
        -e PACT_BROKER_BASE_URL \
